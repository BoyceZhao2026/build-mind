...
- [11. 最终结论](#11-最终结论)
- [12. 相关文档](#12-相关文档)
- [13. Demo 已验证的关键技术决策](#13-demo-已验证的关键技术决策)
  - [13.1 金标准降级为评测与安全基础](#131-金标准降级为评测与安全基础)
  - [13.2 在线核心改为学生动态解法图](#132-在线核心改为学生动态解法图)
  - [13.3 策略驱动的教学回复](#133-策略驱动的教学回复)
  - [13.4 当前会话完整历史上下文](#134-当前会话完整历史上下文)
  - [13.5 两级完结状态](#135-两级完结状态)
  - [13.6 业务逻辑观察台](#136-业务逻辑观察台)
  - [13.7 当前实现边界与后续调研](#137-当前实现边界与后续调研)
...
对于复杂题，“正确理解学生正在做什么”应先于“把它对齐到哪条参考路径”。没有匹配参考步骤不是错误证据；只要学生的局部推理能够独立验证，就可以沿学生的方法继续。

#### 参考解法与步骤图如何生成

> **最新决策（Demo 已验证）**：以下参考解法生成流水线保留为离线评测、按需提示和数学验证能力，不再作为在线会话的前置步骤。在线第一阶段不会把完整参考解法目录提交给学生思路分析模型，以降低锚定效应。
...
如果只把全部聊天记录交给模型自由回复、却没有结构化状态和动作约束，容易出现：
...
- 将数学规范化与意图识别并行执行。
- Demo 阶段提交当前会话完整历史，同时使用结构化状态避免模型重新猜测已确认事实；不能为了延迟静默截断早期消息。
- 对低风险判断采用较轻的模型或确定性工具。
- 缓存常用的非实质性过渡语音。
...
| 内容 | 在线权限 |
| --- | --- |
| 题目事实、约束和可核验答案 | 用于数学验证和安全审核 |
| 一至多条参考解法 | 作为按需检索的弱先验，不规定学生路线 |
| 固定教学问答顺序 | 不进入在线控制逻辑 |

核心原则是：**匹配参考解法是支持证据；没有匹配不是错误证据。** 陌生题不能临时生成一条“伪金标准”后立即当作真相，而应建立带验证状态、可随会话修正的工作解法。
...
最新实现进一步将“临时解法分支”明确为学生动态解法图。学生推理节点不要求存在参考步骤 ID；参考匹配只是可选元数据，不能成为状态推进的必要条件。
...
13. 未匹配参考解法但数学成立的学生方法不会因此被判错。
14. 学生一步覆盖多个推理节点、跳步或换方法时不会被强制拉回固定路径。
15. 当前会话历史消息完整提交，并可在业务逻辑观察台核对提交数量。
16. `solved → reflect → complete` 状态可稳定收敛，进入 `complete` 后不再继续提问。

## 11. 最终结论

这个形态最值得优先验证的，不是“AI 能否像电话一样说话”，而是：

> AI 能否在实时交流中理解学生自己的解题思路、独立验证局部推理，并让语音和黑板共同给出恰到好处的下一步。
...
本章维护从 [Demo 技术选型文档](./DEMO_TECHNICAL_SELECTION_3_0_TO_3_8.md) 进入实现、测试并获得初步验证的关键决策。它是 MVP 后续调研的当前基线；候选方案和历史讨论仍保留在前文，但与本章冲突时以本章为准。

当前技术基线：

| 能力 | 当前决策 |
| --- | --- |
| 客户端 | 桌面 Web，Next.js + TypeScript；暂不做手机端 |
| 题目入口 | 手动上传图片，识别后必须由学生确认 |
| 图片识别 | 通过多模态适配器调用 `qwen3-vl-flash` |
| 教学 LLM | 统一 `LLMAdapter` 封装，当前使用 `qwen-plus`，允许分任务替换和模型对比 |
| STT | 阿里云 Fun-ASR Realtime；浏览器录音转 16 kHz 单声道 WAV 后上传 |
| TTS | 阿里云 CosyVoice，语音、字幕和黑板使用同一轮输出包 |
| 语音轮次 | MVP 使用显式半双工：“点击说话—我说完了—AI 回复” |
| 状态存储 | Demo 保存在本地进程内；生产阶段再引入 SQLite/持久化版本控制 |
| 外部请求 | 服务适配器忽略系统代理，避免本机代理影响阿里云调用 |
| 可观测性 | API、Web 和脱敏模型调用分别写入本地日志 |

### 13.1 金标准降级为评测与安全基础

金标准继续保留，但不再是在线教学的主控制器：

- `evaluation/` 主要用于离线评测、模型选型、版本回归和高风险错误校准。
- 题目事实、约束和内部答案可用于数学验证及答案泄露检查。
- 参考解法只能作为按需检索的弱先验，不能规定学生必须采用的路线或问答顺序。
- 在线第一阶段不向思路分析模型提供完整 `solution_catalog`，先独立理解学生表达。
- 未匹配参考解法不能作为判错或拒绝推进的依据。

这一决策解决了参考路线对模型的锚定问题，同时保留可评测性和安全锚点。

### 13.2 在线核心改为学生动态解法图

在线会话维护题目世界模型和学生动态解法图，而不是使用 `solution_paths[0].steps + current_step_index` 机械推进。
...
每个节点必须能回溯到学生原话。`reference_step_id` 是可选元数据，不是节点身份。系统支持：

- 一轮表达覆盖多个数学主张。
- 学生跳步、回退、自我修正和中途换方法。
- 学生采用线段图、算术法等未收录方法时继续建立节点。
- 局部关系成立时沿学生思路继续；证据不足时只追问缺口。
- 结论正确但理由错误时不自动确认依赖节点。

Demo 真实调用已验证：金标准中仅有方程参考路径时，学生采用线段图法仍可生成无 `reference_step_id` 的已验证节点并继续辅导。
...
状态机生成结构化 `TeachingPolicy`，包含教学动作、当前任务、提示等级、允许信息和禁止信息。回复模型不能自由决定教学进度，也不能获得内部最终答案和完整后续解法。
...
LLM Schema 必须真正随提示提交给模型，不能只在调用端声明 `json_object`；否则字段名和枚举可能漂移，导致状态数据静默丢失。

### 13.4 当前会话完整历史上下文

Demo 阶段不截取大模型历史消息。学生步骤分析和教学回复获得当前会话全部学生/AI 消息，并同时获得：
...
- 学生主张和自我修正。
- 未解决问题与错误认识。
- 当前会话全部教师问题，用于重复问题 Guard。

结构化状态用于提高一致性，而不是替代或截断原始历史。业务观察台显示本轮实际提交的历史消息数量，便于确认上下文是否完整。

生产阶段只有在单次会话触及模型上下文上限时，才引入可回溯摘要、事实账本和分段检索；任何优化都不能静默丢弃早期学生证据。

### 13.5 两级完结状态

学生给出最终解答不等于已经证明理解，因此结束过程分为两级：

```mermaid
stateDiagram-v2
    attempt --> reflect: solved
    reflect --> complete: understanding_verified
    reflect --> reflect: 解释仍不充分
    complete --> [*]
```
...
- `solved`：已经完成解答，进入一次简短的关键关系复述或理解检验。
- `understanding_verified`：解释证据通过，进入 `complete`。
- `complete`：AI 简短肯定并结束，不再提出问题；Web 停用录音和文字输入。

后端将“无”“当前小任务已完成”等旧模型文本防御性转换为 `solved`，不能将其拼接成新的教学任务。

除系统自动完结外，学生可以点击“我已理解，完成本题”主动结束。学生触发表示其选择结束本次辅导，不应伪装成系统已经验证理解；完成记录必须保留 `trigger=student/system`。

两种触发都会根据完整对话和学生动态解法图生成一份总结卡，包括使用的方法、关键数量关系、2～5 条思路步骤和易错提醒。总结用于学生离开本题前的第二次思路梳理：可以沿学生采用的方法整理完整的通用步骤，但不得代算具体结果、给出最终答案或引入另一条内部解法。总结 Guard 会拦截最终答案数值，以及结束后继续提问或布置下一步；模型失败时使用已验证推理节点生成本地兜底总结。

### 13.6 业务逻辑观察台

Demo 页面在 AI 黑板区域提供可展开的业务逻辑观察台，动态展示：

- 学生意图、本轮数学判断、思路连续性和置信度。
- 学生当前方法概括。
- 动态解法图中的主张、原话证据、规范化表达、依赖和验证状态。
- 当前缺口、下一教学目标和状态机教学动作。
- `solution_status` 和本轮提交的历史消息数量。
- 输出 Guard 决策、违规原因与答案泄露状态。

观察台展示可审计的结构化业务决策证据，不展示模型隐藏的内部推理文本。其目标是区分问题发生在理解、数学验证、教学策略还是回复生成层。

### 13.7 当前实现边界与后续调研

当前 Demo 已验证动态思路理解的业务可行性，但尚不能将模型给出的 `verification_status=verified` 视为最终数学事实。下一阶段优先事项是独立验证层：

1. 使用 SymPy 验证算式、方程等价变形和结果代回。
2. 使用确定性题目对象和单位规则验证数量绑定。
3. 对自然语言关系或复杂新方法使用独立验证模型，并与理解模型职责隔离。
4. 验证失败或证据不足时保留 `unverified`，提出一个澄清问题，不推进理解状态。
5. 为替代方法、长距离历史引用、一步覆盖多节点、错误理由得到正确结果以及完结状态建立回归样本。

因此，当前核心仍是：**先理解学生自己的思路，再独立验证局部推理，最后决定最小必要的教学动作。**
