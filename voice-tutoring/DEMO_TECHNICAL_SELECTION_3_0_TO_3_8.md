...
- [5. 3.2 引导式教学状态机](#5-32-引导式教学状态机)
  ...
  - [5.10 学生主动完成与解题思路总结](#510-学生主动完成与解题思路总结)
...
系统完结判断和学生主观感受可能不同。学生认为已经完全理解时，可以点击“我已理解，完成本题”，不需要继续完成状态机安排的追问。两种触发统一进入同一个收口流程：

```mermaid
flowchart LR
    A[系统判断 understanding_verified] --> C[complete]
    B[学生点击完成] --> C
    C --> S[基于完整对话与学生动态解法图生成总结]
    S --> V[Schema 校验或本地兜底]
    V --> P[页面展示解题思路总结卡]
    P --> E[停用本题输入并允许换题]
```
...
总结的目标是作为学生离开本题前的第二次思路梳理，而不是重新讲一遍完整答案：

- 只使用题目、完整会话和学生动态推理节点，以学生已经采用的方法为主线。
- 可以把该方法整理成完整的通用步骤，但不能代算具体结果、给出最终答案或引入另一条内部参考解法。
- 忽略 `rejected` 节点；`unverified` 节点不能写成确定结论。
- 用 2～5 个简短步骤整理学生实际使用的方法。
- 突出关键数量关系，并给出一个与本次过程有关的易错提醒。
- 模型生成失败或 Schema 不合法时，根据已验证节点生成本地兜底总结。
- 总结经过专用 Guard：删除含最终答案数值的步骤；结束语不得继续提问、布置“下一步”或邀请继续计算。
- 完成接口具有幂等性，重复点击返回已生成总结，不重复调用模型。

学生主动完成表示“学生选择结束本次辅导”，不等于系统已经获得 `understanding_verified` 的客观证据。总结卡需保留 `trigger=student/system`，供后续学习记录区分主观完成与系统验证完成。
